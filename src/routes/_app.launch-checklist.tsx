...
function LaunchChecklistPage() {
  return (
    <div className="space-y-8">
      <PageHeader title="Launch Checklist" description="A phased plan that turns your strategy into a real China launch." />
          ...
          <div key={p.name} className="overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--background)] shadow-[var(--shadow-soft)]">
            <div className="flex items-center justify-between gap-4 border-b border-[var(--border)] px-6 py-4">
              <div>
                ...
                <p className="mt-0.5 text-xs text-[var(--muted-foreground)]">{p.items.filter((i) => i.done).length} of {p.items.length} complete</p>
              </div>
              <div className="flex items-center gap-3">
                <div className="h-1.5 w-40 overflow-hidden rounded-full bg-[var(--muted)]">
                  <div className="h-full rounded-full bg-[var(--primary)]" style={{ width: `${p.progress}%` }} />
                </div>
            ...
            <ul className="divide-y divide-[var(--border)]">
              {p.items.map((item) => (
                <li key={item.label} className="flex items-center gap-3 px-6 py-3">
                  {item.done ? (
                    <div className="grid h-5 w-5 place-items-center rounded-full bg-[var(--primary)] text-[var(--primary-foreground)]">

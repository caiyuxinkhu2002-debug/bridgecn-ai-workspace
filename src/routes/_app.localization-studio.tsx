...
import { ArrowRight, Wand2, Copy, FileDown, Save, History as HistoryIcon, Trash2, RotateCw, ShieldCheck, GitCompare, CheckCircle2, Loader2, Sparkles, Square } from "lucide-react";
...
type LocCompliance = { advertising?: string; sensitive?: string; risk?: string; regulation?: string };
...
function LocalizationStudioPage() {
  ...
  const toMarkdown = useCallback(() => {
    ...
    if (displayed.updatedAt) lines.push(`- Generated: ${new Date(displayed.updatedAt).toLocaleString()}`);
    lines.push("");
    ...
    if (displayed.insights.reasoning) {
      ...
      if (displayed.insights.consumer) lines.push(`- Consumer preference: ${displayed.insights.consumer}`);
      if (displayed.insights.seo) lines.push(`- SEO keywords: ${displayed.insights.seo.join(", ")}`);
      if (displayed.insights.platform) lines.push(`- Platform optimization: ${displayed.insights.platform}`);
      if (displayed.insights.cultural) lines.push(`- Cultural adaptation: ${displayed.insights.cultural}`);
    }
    ...
    <div>
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-4">
          {displayed.items.length === 0 && !ai.isRunning ? (
              ...
              <div className="grid divide-y divide-[var(--border)] md:grid-cols-2 md:divide-x md:divide-y-0">
                ...
                <div className="p-5">
                  <div className="mb-2 flex items-center justify-between">
                    ...
                    <button onClick={generate} className="inline-flex items-center gap-1 text-xs font-medium text-[var(--primary)]"><Wand2 className="h-3 w-3" />{t("common.regenerate")}</button>
                  </div>
                  <p className="text-base leading-relaxed">{p.target}</p>
                </div>
              </div>
              <div className="border-t border-[var(--border)] bg-[var(--muted)]/50 px-5 py-2 text-[11px] text-[var(--muted-foreground)]">{tone} · {channel} · {audience} — {p.note}</div>
            </div>
          ))}
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            <div key={s.label} className="rounded-2xl border border-[var(--border)] bg-[var(--background)] p-5 shadow-[var(--shadow-soft)]">
              ...
              <p className="mt-2 text-2xl font-semibold tracking-tight tabular-nums">{s.v ?? "—"}{s.v != null ? <span className="text-sm font-normal text-[var(--muted-foreground)]"> /100</span> : null}</p>
              <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-[var(--muted)]/60">
                <div className="h-full rounded-full bg-[var(--primary)]" style={{ width: `${s.v ?? 0}%` }} />
              </div>
            </div>
          ))}
        </div>
...
function ControlRow({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="flex flex-wrap items-center gap-2">
      <span className="w-32 shrink-0 text-[10px] font-semibold uppercase tracking-wider text-[var(--muted-foreground)]">{label}</span>
...
function ExportBtn({ icon, onClick, children }: { icon: React.ReactNode; onClick: () => void; children: React.ReactNode }) {
  return (
    <button onClick={onClick} className="inline-flex items-center justify-center gap-1.5 rounded-md border border-[var(--border)] px-3 py-2 text-xs font-medium hover:bg-[var(--muted)]/60">
      {icon}{children}
    </button>
  );
}

  ...
  return (
    <div>
      <div className="flex items-end justify-between gap-4">
        ...
        <button onClick={markAllRead} className="h-9 rounded-md border border-[var(--border)] px-3 text-xs font-medium hover:bg-[var(--muted)]">{t("notif.markAll")}</button>
      </div>
      <div className="space-y-6">
        {groups.map((g) => {
          ...
          return (
            <div key={g.key}>
              <h3 className="mb-2 text-[11px] font-semibold uppercase tracking-wider text-[var(--muted-foreground)]">{t(g.labelKey)}</h3>
                ...
                <ul className="divide-y divide-[var(--border)]">
                  {list.map((n) => (
                    <li key={n.id} className="flex items-start gap-3 px-4 py-3">
                      <div className="mt-0.5 grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-[var(--muted)]"><Bell className="h-3.5 w-3.5" /></div>

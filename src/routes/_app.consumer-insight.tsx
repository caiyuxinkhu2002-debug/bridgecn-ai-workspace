...
function ConsumerInsightPage() {
  return (
    <div className="space-y-8">
      <PageHeader title="Consumer Insight" description="Personas, sentiment and behavioral trends across Chinese consumers." />

      <div className="grid gap-4 lg:grid-cols-3">
        {personas.map((p) => (
          <div key={p.name} className="rounded-2xl border border-[var(--border)] bg-[var(--background)] p-6 shadow-[var(--shadow-soft)]">
            <div className="flex items-center gap-3">
              <div className="grid h-12 w-12 place-items-center rounded-full bg-gradient-to-br from-[var(--primary-soft)] to-[var(--muted)] text-sm font-semibold">
            ...
            <div className="mt-5 space-y-2 text-sm">
              <div className="flex justify-between"><span className="text-[var(--muted-foreground)]">Income</span><span>{p.income}</span></div>
              <div className="flex justify-between"><span className="text-[var(--muted-foreground)]">Mindset</span><span className="rounded-full bg-[var(--primary-soft)] px-2 py-0.5 text-[11px] font-medium text-[var(--primary)]">{p.tag}</span></div>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="rounded-2xl border border-[var(--border)] bg-[var(--background)] p-6 shadow-[var(--shadow-soft)]">
        <div className="mb-4 flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold">Trend signals</h3>
        </div>
            ...
            <li key={s.label} className="flex items-center justify-between py-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-[var(--muted-foreground)]">{s.source}</p>
                </div>
              </div>
              <span className="rounded-full bg-[oklch(0.96_0.04_150)] px-2.5 py-0.5 text-xs font-medium text-[oklch(0.42_0.12_150)]">
                {s.sentiment}
              </span>
            </li>
          ))}

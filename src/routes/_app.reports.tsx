  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const list = await listReports({
        data: { workspaceId: activeWorkspace.id, projectId: activeProject?.id || null },
      });
      setRows(list);
    } catch (e) {
    ...
    <div>
      ...
      <div className="overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--background)] shadow-[var(--shadow-soft)]">
        <ul className="divide-y divide-[var(--border)]">
          {loading ? (
            <li className="px-5 py-10 text-center text-xs text-[var(--muted-foreground)]">
            ...
            filtered.map((r) => (
              <li
                key={r.id}
                className="flex items-center gap-4 px-5 py-4 transition-colors hover:bg-[var(--muted)]/60"
              >
                ...
                <div className="min-w-0 flex-1">
                  <button
                    onClick={() =>
                      router.navigate({ to: "/report", search: { reportId: r.id } as never })
                    }
                ...
                <span
                  className={`rounded-full px-2.5 py-0.5 text-[11px] font-medium ${r.status === "Ready" ? "bg-[oklch(0.96_0.04_150)] text-[oklch(0.42_0.12_150)]" : "bg-[var(--muted)] text-[var(--muted-foreground)]"}`}
                >
                  {r.status === "Ready" ? t("reports.status.ready") : t("reports.status.draft")}
                </span>
                <button
                  onClick={() => onShare(r.id, r.title)}
                  className="grid h-8 w-8 place-items-center rounded-md hover:bg-[var(--background)]"
